...
layout( set = 0, binding = 2 ) uniform sampler2D shadowMask;
layout( set = 0, binding = 3 ) uniform sampler2D depth;

layout( set = 0, binding = 4 ) uniform Views { View views[MAX_VIEWS]; };
layout( set = 0, binding = 5 ) uniform InverseViews { InverseView inverseViews[MAX_VIEWS]; };
layout( set = 0, binding = 6 ) uniform Lights { Light lights[MAX_LIGHTS]; };
...
void main()
{
   ...
   const vec3 viewDir   = normalize( mainView.pos.xyz - worldPos.xyz );
